...
void PrintActiveConfiguration()
{
   ...
   PrintFormat("   [Risk Governor] Target Risk: $%.2f | Absolute Hard Cap: $%.2f | Min Lot: %.2f | Max Lot Clamp: %.2f",
               InpMaxRiskUSD, InpMaxRiskHardCapUSD, InpMinLotSize, InpMaxLotSize);
   ...
   Print(ToggleState(InpUseConcurrencyCap),   "InpUseConcurrencyCap (Max=" + IntegerToString(InpMaxConcurrentTrades) + ", MaxSameDir=" + IntegerToString(InpMaxPositionsPerDir) + ")");
...
int OnInit()
{
   ...
   EventSetTimer(1);          // Start a 1-second interval timer for real-time dashboard updates
...
{
   EventKillTimer();          // Stop the 1-second interval timer
   DecisionLogDeinit();
   OutcomeTrackerDeinit();
   if(InpUseAdvancedModel)
   {
      // Release is handled automatically by CGRUAdvancedFilter destructor
   }
   else
   {
      g_gru76.Release();
...
//+------------------------------------------------------------------+
//| Expert timer — real-time GUI updates                           |
//+------------------------------------------------------------------+
void OnTimer()
{
   CheckExitContractTick(); // Real-time 1-second trailing stop ratchet guard
   RefreshTodayStats();
   SyncDashboardState();
   DashboardRefresh();
}
...
void OnTick()
{
   ...
   RefreshTodayStats();          // Update stats on every tick immediately when positions close
   SyncDashboardState();
   RenderGnnLines();
   DashboardRefresh();
}